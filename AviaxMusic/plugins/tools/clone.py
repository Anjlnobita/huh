...
from AviaxMusic import CLONE_OWNERS
...
async def clone_txt(client, message):
    if len(message.command) > 1:
        ...
        try:
            app = Client(bot_token, API_ID, API_HASH, bot_token=bot_token, plugins=dict(root="AviaxMusic/plugins"))
            await app.start()
...
async def restart_bots():
    ...
    try:
        ...
        async def restart_bot(bot):
            bot_token = bot["token"]
            app = Client(bot_token, API_ID, API_HASH, bot_token=bot_token, plugins=dict(root="AviaxMusic/plugin"))
            try:
                await app.start()
